...
void test_time(FILE * f, config c,vector<bb> &bb_vec ,vector<char *> &insn_pool){
    ...
    pro_1.push_back(InsnFactory::create_s_memtime(c.memtime_sgpr_1,insn_pool));
    ...
    set_writeback_pro_ep(pro_3,ep_3,c.addr_sgpr,c.addr_vgpr,c.memtime_sgpr_1,c.acc_vgpr,insn_pool);
    set_writeback_pro_ep(pro_3,ep_3,c.addr_sgpr,c.addr_vgpr,c.memtime_sgpr_2,c.acc_vgpr,insn_pool);
    
    //set_writeback_pro_ep(pro_3,ep_3,c.addr_sgpr,c.addr_vgpr,36,c.acc_vgpr,insn_pool);
    //set_writeback_pro_ep(pro_3,ep_3,c.addr_sgpr,c.addr_vgpr,c.addr_sgpr+1,c.acc_vgpr,insn_pool);
...
int main(int argc, char **argv){
    ...
    for (const auto & bbptr : (*fit)->blocks()){
        cout << std::hex << bbptr->start() << " " << bbptr->end() << endl;
    ...
    test_time(fp,c,vec_bb,insn_pool);
