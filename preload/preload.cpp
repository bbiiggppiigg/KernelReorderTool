...
void print_result_v2(uint32_t * records, const char * kname ,uint32_t num_warps, uint32_t num_branches, uint64_t no_threads){
  ...
  for ( uint64_t index =0 ; index < 256 && index < no_threads ; index ++){
    fprintf(fdata,"%lu: %x\n",index,records[index]);
  }
  fprintf(fdata,"done\n");
}
    ...
    hipStream_t stream)
{
  ...
  if(c){
    ...
    uint64_t warps_per_block =  ( blockDim.x * blockDim.y * blockDim.z + 63 ) / 64;
    uint64_t no_warps = gridDim.x * gridDim.y * gridDim.z * warps_per_block;

    uint32_t num_branches = c->branch_count; // TODO:

    uint64_t data_size = no_warps * 4 * 64;
    uint32_t no_records = data_size / sizeof(uint32_t);
    uint64_t no_threads = blockDim.x * blockDim.y * blockDim.z * gridDim.x * gridDim.y * gridDim.z;

    kernel_launch_tracker * klt_ptr;
    
    fprintf(fdebug,"looking for memory of size %lu\n",data_size);
    bool found = false;
    for ( int i =0; i < mempool.size() ; i ++){
      klt_ptr = mempool[i]; 
      fprintf(fdebug,"Iterating  memory slot[%d] at 0x%p, size = %u\n",i,klt_ptr->data_h, klt_ptr->avail_size);
      if(klt_ptr->status == 2 && klt_ptr->avail_size >= data_size){
        klt_ptr = mempool[i]; 
        found = true;
        fprintf(fdebug,"Reusing memory at 0x%p, avail size = %u\n",klt_ptr->data_h, klt_ptr->avail_size);
        break;
      }
    }
    hipError_t hip_ret;
    if(found == false){
      fprintf(fdebug,"Can't reuse memory , allocating new memory of size %lu\n",data_size);
      ...
      fprintf(fdebug,"memory allocated from [%p,%p]\n",klt_ptr->data,klt_ptr->data+data_size);
    ...
    fprintf(fdebug,"WARPS_PER_BLOCK = %lu, NO_WARPS = %lu, NUM_BRANCHES = %u\n", warps_per_block, no_warps, num_branches);
    ...
    *(newArgs+c->kernarg_num+5) = &(blockDim.y);
    ...
    print_result_v2 (klt_ptr->data_h, c->name.c_str() , no_warps, klt_ptr->num_branches, no_threads);
    klt_ptr->status=2;
