...
for ff in *; do
    ...
    $BIN/update_kd $ff
    #LD_LIBRARY_PATH=/home/wuxx1279/bin/dyninst-main/lib:$LD_LIBRARY_PATH DYNINST_DEBUG_PARSING=1 OMP_NUM_THREADS=1 KR_DEBUG=1 $BIN/empty $ff $ff.config.ini  
    ...
    #LD_LIBRARY_PATH=/home/wuxx1279/bin/dyninst-amdgpu-codegen/lib:$LD_LIBRARY_PATH DYNINST_DEBUG_PARSING=1 OMP_NUM_THREADS=1 KR_DEBUG=1 $BIN/preload_global_v3 $ff $ff.config.ini  
    ...
    llvm-objcopy --update-section=.text=memtime.text $ff # remove the original note section
    #~/KR/bin/update_text_phdr $ff  &> update_text.log
    #llvm-objcopy --set-section-flags=.old.text=readonly $ff 
    #llvm-objcopy --set-section-flags=.text=alloc,code,readonly $ff 
